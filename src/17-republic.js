...
const MANDATES={
  merchants:[
    {id:"m_tax",label:"Lighten the burden on trade",check:S=>["none","light"].includes(S.taxRate)},
    {id:"m_surplus",label:"Balance the books",check:S=>netIncome(S)>=0},
    {id:"m_build",label:"Build what commerce needs",check:S=>S.development>=(S.rep.devAtStart+4)}],
  peasantry:[
    {id:"p_tax",label:"Lift the taxes off the land",check:S=>["none","light","moderate"].includes(S.taxRate)},
    {id:"p_bread",label:"Keep the country quiet and fed",check:S=>S.stability>=55},
    {id:"p_mood",label:"Govern for the common people",check:S=>S.facs.peasantry.mood>=55}],
  officers:[
    {id:"o_arms",label:"Restore the strength of the army",check:S=>S.military>=(S.rep.armsAtStart+6)},
    {id:"o_mood",label:"Keep faith with the officer corps",check:S=>S.facs.officers.mood>=55}],
  clergy:[
    {id:"c_faith",label:"Honour the faith of the realm",check:S=>S.facs.clergy.mood>=58},
    {id:"c_order",label:"Hold the realm to good order",check:S=>S.stability>=55}],
  aristocracy:[
    {id:"a_priv",label:"Respect the ancient privileges",check:S=>S.facs.aristocracy.mood>=55},
    {id:"a_order",label:"Keep property safe",check:S=>S.stability>=52}],
  provinces:[
    {id:"v_loyal",label:"Govern the country as well as the capital",check:S=>provinceLoyaltyAvg(S)>=55},
    {id:"v_works",label:"Carry the roads out to the provinces",check:S=>S.development>=(S.rep.devAtStart+3)}],
  workers:[
    {id:"w_mood",label:"Answer the demands of labour",check:S=>(S.facs.workers&&S.facs.workers.mood>=55)},
    {id:"w_works",label:"Put the unemployed to work",check:S=>S.development>=(S.rep.devAtStart+4)}],
  reformers:[
    {id:"r_reform",label:"Carry a further measure of reform",check:S=>(S.reforms||[]).length>(S.rep.reformsAtStart)},
    {id:"r_legit",label:"Govern by consent, visibly",check:S=>legitimacy(S)>=58}]
};
function newMandate(S,bloc){
  ...
  return out.map(m=>({id:m.id,label:m.label}));
}
...
function installRepublic(S,fromJunta){
  ...
  if(!S.gov.institutions.some(i=>i.composition==="national")){
  ...
  S.regime="republic";
  S.gov.crown.titleBase="President";
...
function mandatePanel(S){
  ...
  return `<div class="press"><div class="gt">The Mandate</div>
    ${(S.rep.mandate||[]).map(m=>{const done=mandateCheck(S,m.id);
      return `<div class="prow ${done?"":"warn"}"><span class="pn">${esc(m.label)}</span><span class="pw">${done?"kept":"outstanding"}</span></div>`;}).join("")}
