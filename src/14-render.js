...
function familyPanel(){
  const rows=[]; const h=heirOf(S);
  const order={spouse:0,child:1,childspouse:2,grandchild:3,sibling:4,dowager:5,uncle:6,kin:7};
  const fam=S.family.filter(p=>p.rel!=="former"&&(p.alive||p.rel==="spouse"||p.rel==="child"||(p.diedTurn!=null&&S.turn-p.diedTurn<=2))).slice().sort((a,b)=>((order[a.rel]!=null?order[a.rel]:9)-(order[b.rel]!=null?order[b.rel]:9))||b.age-a.age);
  fam.forEach(p=>{ const tr=traitShown(p);
    rows.push(`<div class="person ${p.alive?"":"dead"}"><span class="pr">${relLabel(p)}</span>
    <span>${esc(p.name)}, ${p.age}</span>${tr?`<span class="trait ${tr.sure?"sure":""}">${tr.sure?"":"“"}${esc(tr.text)}${tr.sure?"":"”"}</span>`:""}${p.job?`<span class="office">${esc(((typeof ROLES!=="undefined"?ROLES:[]).find(r=>r.id===p.job)||{}).name||"")}</span>`:""}${p.love?`<span class="office" style="color:#c76b8a">love match</span>`:""}${h&&h.id===p.id?`<span class="heirmark">— heir</span>`:""}${S.rival&&S.rival.id===p.id?`<span class="rival">— rival claimant</span>`:""}${p.exiled?`<span class="rival" style="color:var(--dim)">— exiled</span>`:""}</div>`); });
  ...
  const el=h(`<div><div class="grid">
    <aside class="side">
      ...
      <div class="budget">
        ...
        <div class="bx"><span class="bk">Net / yr</span><span class="bv ${netCls}">${net>=0?"+":"−"}${Math.abs(net)}</span></div>
...
function renderAdvance(){
  ...
  const wkHtml=wks.map(w=>{
    const aff=S.treasury-w.gold>-(40+(S._creditBonus||0));
    return `<button class="choice adv-works" data-buywork="${w.id}" ${aff?"":"disabled"}>
      <div class="cl"><span class="mk">⌂</span><span class="lbl">${esc(w.name)}</span></div>
      <div class="ch">${esc(w.blurb)}</div>
      <div class="costs"><span class="chip down">−${w.gold} gold</span><span class="chip know">+${w.know.toFixed(2)} knowledge/yr, forever</span>
...
function doBuyWork(id){
  ...
  S.treasury-=w.gold; S.works.push(w.id);
  for(const k in (w.fac||{})) S.facs[k].mood=clamp(S.facs[k].mood+w.fac[k]);
  if(w.effect)w.effect(S);
  S.chronicle.push({year:S.year,text:`In ${S.year}, ${w.name.replace(/^The /,"the ")} was founded at the crown's charge.`});
  S.result={out:`${w.blurb}\n\nIt will yield ${w.know.toFixed(2)} knowledge a year for as long as ${S.nation} stands.`,fortune:null,next:"advance"};
...
function renderCourt(){
  const tabs=`<div class="tabs">
    ...
    <button data-tab="dynastic" class="${S.tab==="dynastic"?"on":""}">Dynasty</button>
  </div>`;
  let body="";
  if(S.tab==="govern"){
    ...
    const card=(a,idx)=>{
      const aff=affordable(S,a);const chips=costChips(adjCost(S,a),a.fac,a.consent);
